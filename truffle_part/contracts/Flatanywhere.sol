...
contract Flatanywhere {
  ...
  */
  ...
  function CreateSmartLock(uint256 serialNum, uint256 _unitPrice, uint256 _startTime, uint256 _endTime, uint256 _now)
    external
  ...
  function UpdateSmartLock(bytes32 SLID, uint256 newUnitPrice, uint256 newStartTime, uint256 newEndTime)
    external
    returns (bool success)
  {
    require(ExistSmartLock(SLID) == true, "ExistSmartLock: SmartLock Not Exist!");
    require(OwnSmartLock(SLID, msg.sender) == true, "OwnSmartLock: Current user does not own this lock!");
    SmartLocks[SLID].unitPrice = newUnitPrice;
    if (newStartTime >= newEndTime) {
      success = false;
    } else {
      success = true;
      SmartLocks[SLID].startTime = newStartTime;
      SmartLocks[SLID].endTime = newEndTime;
    }
    emit UpdateLock(SLID, success);
  }
  ...
  */
  ...
  function unlock(bytes32 SLID)
    external
  {
    emit Newunlock(msg.sender, SmartLocks[SLID].serialNum, now);
  }
