...
function bind() {
    ...
    if ($('input[type="checkbox"]')[0].checked == true) { // choose public mode
        if ($("#resultAddr")[0].value == "") {
            alert("Address is empty! Please move the red pointer in the map to the location of lock.");
            return;
        }
        var inputBox = {
            "#stid": "Start time",
    ...
    $("#submitRow").html("");
    $("#submitRow").append('<div class="progress"><div class="indeterminate"></div></div>'); // 无限读取进度条
