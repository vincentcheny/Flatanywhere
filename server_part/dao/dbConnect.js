...
function select_SmartLock_Store(client, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      "select SLID, unit_price, owner, start_time, end_time, address, intro from SmartLock WHERE isShown=true AND end_time > NOW()",
...
function select_SmartLock_by_Owner(client, owner, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'select SLID, nickname, currentuser, isShown, start_time, end_time, unit_price, address from SmartLock WHERE owner="' +
        owner +
        '" ORDER BY isShown ASC, end_time ASC, nickname ASC',
      function(err, result) {
        if (err) throw err;
        else callback(result);
        connection.release();
...
function select_SmartLock_by_CurrentUser(client, currentUser, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'select s.SLID, s.nickname, s.currentuser, s.isShown, s.unit_price, s.address from SmartLock s JOIN Deal d ON s.SLID=d.SLID WHERE s.owner <> "' +
        currentUser +
        '" AND s.currentuser="' +
        currentUser +
        '" AND d.isCheckedIn=true',
      function(err, result) {
        if (err) throw err;
        else callback(result);
        connection.release();
...
function insert_Deal(
  ...
  client.getConnection(function(err, connection) {
    connection.query(
      "insert into Deal (DEALID, SLID, seller_Addr, buyer_Addr, total_Amount, check_in_time, check_out_time, isRead, isWithdraw, isCheckedIn) value(?,?,?,?,?,?,?,?,?,?)",
      [
        DEALID,
        SLID,
        sellerAddr,
...
function select_Deal_Unread(client, currentUser, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'select COUNT(*) AS count from Deal d where d.seller_Addr="' +
        currentUser +
        '" AND d.isRead=false',
      function(err, result) {
        if (err) throw err;
...
function select_Info_BuyerView(client, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      "select s.SLID, d.check_in_time, d.check_out_time from Deal d right JOIN SmartLock s ON d.SLID = s.SLID WHERE s.isShown = true AND s.end_time > NOW() ",
...
function select_Info_SoldLockView(client, currentUser, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'SELECT d.DEALID FROM Deal d WHERE d.seller_Addr = "' +
        currentUser +
        '" AND d.isWithdraw=false AND d.check_in_time <= NOW()',
      function(err, result) {
        if (err) throw err;
        else callback(result);
        connection.release();
...
function select_Info_MyLockFutureView(client, owner, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'select d.SLID, MAX(d.check_out_time) AS last_check_out_time FROM (select check_out_time, SLID from Deal WHERE seller_Addr="' +
        owner +
        '" ORDER BY check_out_time DESC) AS d GROUP BY d.SLID',
...
function select_Info_BoughtLockFutureView(client, owner, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'select s.SLID, s.nickname, s.unit_price, s.address, d.check_in_time, d.DEALID FROM SmartLock s JOIN Deal d ON s.SLID = d.SLID WHERE d.buyer_Addr="' +
        owner +
        '" AND d.isWithdraw=false AND d.isCheckedIn=false ORDER BY d.check_in_time ASC',
      function(err, result) {
        if (err) throw err;
        else callback(result);
        connection.release();
...
function update_Deal_Unread(client, currentUser, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'UPDATE Deal d SET d.isRead=true WHERE d.seller_Addr="' +
        currentUser +
        '"',
      function(err) {
        if (err) {
...
function update_Info_Renting(client, SLID, DEALID, currentUser, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'UPDATE Deal SET isCheckedIn = true WHERE DEALID="' + DEALID + '"',
      function(err) {
        if (err) {
        ...
        connection.query(
          'UPDATE SmartLock SET currentUser ="' +
            currentUser +
            '" WHERE SLID = "' +
            SLID +
            '"',
...
function update_SmartLock_hideStore(client, SLID, callback) {
  client.getConnection(function(err, connection) {
    ...
    connection.query(
      'UPDATE SmartLock SET isShown = false WHERE SLID = "' + SLID + '"',
      function(err) {
        if (err) {
...
function update_Deal_Withdraw(client, DEALID, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'UPDATE Deal SET isWithdraw=true WHERE DEALID = "' + DEALID + '"',
      function(err) {
        if (err) {
...
function select_SmartLock_Deletable(client, SLID, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'SELECT COUNT(*) AS numOfDeletable FROM Deal WHERE SLID = "' +
        SLID +
        '" AND check_out_time>=CURRENT_DATE()',
      function(err, result) {
        if (err) throw err;
        else callback(result);
        connection.release();
...
function delete_SmartLock_by_SLID(client, SLID, callback) {
  client.getConnection(function(err, connection) {
    connection.query(
      'DELETE FROM SmartLock WHERE SLID = "' + SLID + '"',
      function(err) {
        if (err) {
